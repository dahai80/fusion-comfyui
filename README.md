...
**Qwen-Image t2i node compatibility for AICF** (v0.2.14, PR #68, fusion-mlx v0.8.51 PR #698): the AICF `qwen-2512-t2i.json` workflow was blocked at the ComfyUI validation gate (HTTP 400) — unregistered node types (`UnetLoaderGGUF`, an empty `LoraLoaderModelOnly` enum) and unrecognized model filenames (`qwen-image-2512-Q4_K_M.gguf`, `qwen_2.5_vl_7b_nvfp4.safetensors`, `qwen_image_vae.safetensors`). Four passthrough nodes were added in `passthrough.py`: `UnetLoaderGGUF` (returns a `FusionModelWrapper` — no-op, the engine resolves the model from `wrapper.model_name`), `LoraLoaderModelOnly` (merges `folder_paths.get_filename_list("loras")` + a fallback list so an empty loras dir no longer produces an empty enum — the validation-400 root cause; mflux ignores the LoRA, the 4-step schedule is baked into the variant config), `ModelSamplingAuraFlow` + `CFGNorm` (no-op passthroughs). All four register in `NODE_CLASS_MAPPINGS` + `_native_overrides` (`__init__.py`); the qwen gguf/clip/vae filenames land in the `loaders.py` `_KNOWN_*` dropdown lists. Routing: `_map_unet_name_to_model_name` maps `qwen-image*` → `mlx-community/Qwen-Image-2512-4bit` (24GB, mflux-compatible, pre-quantized); `_infer_model_type` returns `image` (not video); `_should_use_staged` routes qwen-image txt2img to the **monolith** `engine.generate()` path (the staged `encode_text` hardcodes `Flux2PromptEncoder.encode_prompt` upstream, and the qwen tokenizer has no `chat_template` → `ValueError` — the monolith calls mflux `generate_image` internally which works). Two AICF H3-compat tweaks landed with it: `_sampler_constants` adds the `res_multistep` sampler alias (→ `unipc`; the H3 engine uses its own internal flow schedule and ignores the solver name) and `h3.py` `EmptyMiniMaxH3LatentAV` moves `batch_size` from required to optional (AICF omits it, default 1). TDD: 13 qwen tests (`test_qwen_t2i_compat.py`) + 6 h3 tests (`test_h3_loader_compat.py`). Upstream: fusion-mlx PR #698 registers `qwen_image`/`qwen_image_edit` in `ImageGenEngine.VARIANT_MAP` + `_infer_variant` (check `qwen-image-edit` before `qwen-image` since both contain the substring, before the flux fallback). Real-model e2e: `qwen-2512-t2i.json` through ComfyUI `/prompt` (localhost:11445) = PASS, 375.7K PNG; engine-chain `ImageGenEngine.generate` → mflux `QwenImage` = PASS (1024×1024, mean 101.1, std 85.3, 28.5s). 109 tests pass, ruff clean. AICF `.env.local` points `COMFYUI_BASE_URL` → `localhost:11445` + `COMFYUI_WORKFLOWS_DIR` → its own `src/lib/comfyui/workflows/atomic` (env-only; AICF code untouched).
